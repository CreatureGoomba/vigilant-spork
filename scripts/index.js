...
$(document).ready(function() {
  ...
  (function datepickerInit() {
    ...
    dp.year
      .init()
      .activate($.inArray(initial ? initial.year : d.getFullYear(), years));
  ...
  boneage.initHints = function() {
    var bmh = boneage.male.hint,
      bfh = boneage.female.hint;
    ...
    if (SlyCarousel.initialized) {
      if (pt.sex === "male" || pt.sex === "female") {
        ...
        var i,
          len,
          closestAge = null,
          closestAgeIndex = null;
        for (i = 0, len = ref[pt.sex].ages.length; i < len; i++) {
          if (
            Math.abs(pt.age - ref[pt.sex].ages[i]) <
              Math.abs(pt.age - closestAge) ||
            closestAge === null
          ) {
  ...
  boneage.setHint = function() {
    ...
    if (
      $("#cbHints").is(":checked") &&
      !$("#wrap").next("div.popover:visible").length
    ) {
      // ... then show popover
      popover.show();
    }
  ...
  ref.getAge = function() {
    ...
    if (pt.age !== undef && (pt.sex === "male" || pt.sex === "female")) {
      for (i = 0, len = ref[pt.sex].BFagesCalc.length; i < len; i++) {
        if (
          ref.age === undef ||
          Math.abs(ref[pt.sex].BFagesCalc[i] - pt.age) <
            Math.abs(ref.age - pt.age)
        ) {
          ref.age = ref[pt.sex].BFagesCalc[i];
        }
      }
    } else {
      ref.age = undef;
  ...
  ref.getStDev = function() {
    ...
    ref.stdev = ref[pt.sex].BFstdevs[ref[pt.sex].BFagesCalc.indexOf(ref.age)];
